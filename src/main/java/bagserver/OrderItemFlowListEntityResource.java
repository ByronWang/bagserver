		...
		try {
			stepStore.save(new TransactionCaller() {
				...
				public void exec(DataSession session) throws Exception {
					...
					step.put("ActionName", actionStore.get((Long) step.get("ActionID")).get("Name"));
					...
					case 2:// "选中买手"
						...
						step.put("StatusName", statusStore.get((Long) step.get("StatusID")).get("Name"));
						...
						item.put("ActionName", actionStore.get((Long) item.get("ActionID")).get("Name"));
						break;
					case 9:// "开始支付"
//						session.add(stepStore, step);
...
//                              Preconditions.checkArgument(amount.equals(result.getSettleAmount()));
								...
								stepPayConfirmed.put("StatusName", statusStore.get((Long) stepPayConfirmed.get("StatusID")).get("Name"));
